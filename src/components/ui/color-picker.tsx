...
const HEX_MATCH_REGEX = /^#([a-fA-F0-9]{3}|[a-fA-F0-9]{6})$/;
const RGB_MATCH_REGEX =
  /^rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*([\d.]+))?\s*\)$/;
const HSL_MATCH_REGEX =
  /^hsla?\(\s*(\d+)\s*,\s*(\d+)%\s*,\s*(\d+)%\s*(?:,\s*([\d.]+))?\s*\)$/;
const HSB_MATCH_REGEX =
...
function parseRgbColor(rgbMatch: RegExpMatchArray): ColorValue {
  return {
...
function parseHslColor(hslMatch: RegExpMatchArray): ColorValue {
...
function parseHsbColor(hsbMatch: RegExpMatchArray): ColorValue {
...
function parseColorString(value: string): ColorValue | null {
  ...
  if (trimmed.startsWith("#")) {
    const hexMatch = trimmed.match(HEX_MATCH_REGEX);
    if (hexMatch) {
  ...
  const rgbMatch = trimmed.match(RGB_MATCH_REGEX);
  if (rgbMatch) {
    return parseRgbColor(rgbMatch);
  }

  // Parse hsl/hsla colors
  const hslMatch = trimmed.match(HSL_MATCH_REGEX);
  if (hslMatch) {
    return parseHslColor(hslMatch);
  }

  // Parse hsb/hsba colors
  const hsbMatch = trimmed.match(HSB_MATCH_REGEX);
  if (hsbMatch) {
    return parseHsbColor(hsbMatch);
  }
...
function createColorPickerStore(
  ...
  stateRef: RefObject<ColorPickerStoreState>,
...
function useColorPickerStoreContext(consumerName: string) {
  const context = use(ColorPickerStoreContext);
  if (!context) {
    throw new Error(
...
const ColorPickerStoreContext = createContext<ColorPickerStore | null>(null);
...
interface ColorPickerRootProps
  extends Omit<ComponentProps<"div">, "onValueChange">,
    Pick<
...
function ColorPickerRoot(props: ColorPickerRootProps) {
  ...
  const storeCallbacks = useMemo<ColorPickerStoreCallbacks>(
    () => ({
...
interface ColorPickerAreaProps extends ComponentProps<"div"> {
  asChild?: boolean;
}
...
interface ColorPickerSwatchProps extends ComponentProps<"div"> {
  asChild?: boolean;
}
...
interface ColorPickerEyeDropperProps extends ComponentProps<typeof Button> {}
